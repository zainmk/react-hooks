...
- Local storage to save state across window refreshes
- Increase performance during React useState/useReducer with lazy state initialization, by passing a reference to function instead of value. Only runs during initial render.
- If no dependency array given, useEffect runs on every re-render (limit for performance). If empty dependency array, useEffect runs on only initial render. Shallow comparison (===).
- Custom Hooks are just regular custom reusable functions, that use other React hooks within. React Syntax (eslint) requires them to start with 'use...'
- React Flow = Mount (lazy init) -> Update (render, layout, paint, effects) -> UnMount 
