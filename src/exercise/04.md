...
- use spread operator for deep copies (beware nested objects)
- keep state close to the actual data value.
- instead of directly mutating state referenced variables (without the setters, make a copy then reassign)
- performance optimization = lazy init in useState, tight dependency array in useEffect
