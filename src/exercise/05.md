...
- Use the 'ref' attribute to directly connect and acess a DOM node
- With the 'ref', the library knows where to apply the styles
- References to DOM nodes need to be explicitly removed otherwise they exist post program end and lead to memory leaks (if being accessed by external libraries)
- Access DOM through side effects (useEffects)
- Class vs Functional components
  - component mount = useEffect with empty dependency array
  - component unmount = return in above useEffect
  - return for render is return for component
