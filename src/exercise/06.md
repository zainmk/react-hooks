...
- Backend requests count as effects
- Can only return a cleanup function for useEffects, therefore define async/await function then call the function.
- Either use the async/await/execute or the .then syntax.
- Error handling with promises can be handled using the second parameter of .then OR using the .catch for error handling on both the promise and function being called on it
- consider using custom strings to track promise status, updated through dispatch, instead of state booleans
- automatic batching will allow async to rerender along with other states
- consider keeping all state variables within a single state object. ensure you do not overwrite the entire object when setting. can use spread syntax on current state to fill.
- Error Boundaries need to be class defined for now. They can be used to apply error handling around html elements themselves, useful to use around components which may share errors and can capture both runtime and request errors. Error boundaries explicitly call 'getDerivedStateFromError'.
- Giving the ErrorBoundary component a unqiue key prop, uniquely defines it so it resets entirely when the app re-renders from change in pokemonName.
- resetKeys prop is a list of values that trigger a reset in the ErrorBoundary
