...
- lifting state is giving visibility of the prop in a component, to its siblings. Keep consistent and stlying of prop drilling to not get confused, passing props through componenets
- onChange can be defined at any level it makes most sense ('event' is visible from parent)
- state colacating is condensing a state into a component, when it does not need to be visible (therefore relevant) to the intended's sibling components. Important to do as it keeps code clean.
- recall, proper state control can help limit component renders and therefore help with speed
