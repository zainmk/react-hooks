...
- When the app reruns (from a varaible state change) the entire component function will run again. This happens when the setter method for useState is called.
- The component can be 'unounted' where the function no longer runs on rerenders 
- Controlled vs Uncontrolled componenets. Connecting the 'input - value' attribute to the useState variable, and the 'input - onChange' to the useState setter function = controls the componenet tightly to the React state. Controlled allows checking for every input from user, while uncontrolled allows accessing the HTMl base DOM (if need to merge with other code). Uncontrolled would have a React Ref to the input form, picked up by the HTML DOM.
- When controlled, note 'null' values that could exist in both states (DOM nodes or ReactJS variable)
